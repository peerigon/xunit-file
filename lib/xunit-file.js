...
function writeTest(fd, test) {
  ...
  if ('failed' == test.state) {
    var err = test.err;
    var diff = base.hideDiff || typeof Base.generateDiff === 'undefined'
      ? ''
      : '\n' + Base.generateDiff(err.actual, err.expected);
    appendLine(fd, tag('testcase', attrs, false, tag('failure', { message: escape(err.message) }, false, cdata(diff + err.stack))));
